    ...
    <div className="min-h-svh flex items-center justify-center p-6">
      <div className="w-full max-w-[480px] reveal">
        <div className="flex justify-end mb-4">
          <button onClick={() => nav('/', { replace: true })} className="text-[var(--ink-3)] text-[14px] font-bold">Skip</button>
        </div>

        <div className="text-center mb-8">
          <div className="font-head font-bold text-[34px] text-[var(--ink)] leading-[1.1] tracking-[-.02em]">
            Earn in 3 taps.<br />Tiny tasks, real money.
          </div>
          <div className="text-[var(--ink-3)] text-[15px] font-semibold mt-3 leading-[1.5]">
        ...
        <div className="flex flex-col gap-3">
          {SLIDES.map((s, idx) => (
            ...
            >
              ...
              <div>
                <div className="text-[var(--ink)] text-[16px] font-extrabold font-head">{s.title}</div>
                <div className="text-[var(--ink-4)] text-[13px] font-semibold mt-[2px]">{s.body}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8">
          <div className="flex justify-center gap-[6px] mb-4">
            {SLIDES.map((_, idx) => (
              <span key={idx} className={`h-[6px] rounded-full transition-all ${idx === i ? 'w-6 bg-[var(--accent)]' : 'w-[6px] bg-[var(--fill-2)]'}`} />
            ))}
          </div>
          <button
