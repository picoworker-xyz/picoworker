    ...
    <div className="min-h-svh flex flex-col">
      <header className="border-b border-[var(--line)]">
        <div className="app-container h-16 flex items-center justify-between">
          ...
          <button onClick={() => nav('/login')} className="text-[var(--ink-3)] text-[14px] font-bold hover:text-[var(--ink)]">Log in</button>
        </div>
      </header>
      ...
      <div className="flex-1 app-container py-10 lg:py-16 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-12 items-center">
        ...
        <div className="hidden lg:block">
          <div className="inline-flex items-center gap-2 px-3 py-[7px] rounded-full bg-[rgba(139,108,255,.12)] border border-[rgba(139,108,255,.3)] mb-6">
            ...
            <span className="text-[var(--ink-2)] text-[12.5px] font-bold">For businesses & creators</span>
          </div>
          <h1 className="font-head font-bold text-[44px] leading-[1.08] tracking-[-.02em] text-[var(--ink)]">Reach real people, fast.</h1>
          <p className="text-[var(--ink-3)] text-[16px] font-medium mt-4 max-w-[420px] leading-[1.6]">
        ...
        {/* form */}
        <div className="w-full max-w-[440px] mx-auto rounded-[24px] bg-[var(--card)] border border-[var(--line)] p-7">
          <div className="font-head font-bold text-[22px] text-[var(--ink)]">Create a business account</div>
          <div className="text-[var(--ink-3)] text-[14px] font-semibold mt-1 mb-5">Launch your first campaign in minutes.</div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {CATEGORIES.map((c) => (
              <button key={c} onClick={() => setCategory(c)} className={`py-[11px] rounded-[12px] text-[13px] font-head font-extrabold border ${category === c ? 'bg-[var(--accent)] text-[var(--accent-ink)] border-transparent' : 'bg-[var(--fill)] text-[var(--ink-2)] border-[var(--line)]'}`}>
                {c}
              </button>
            ))}
          </div>
          ...
          <button onClick={() => setAgree((v) => !v)} className="flex items-center gap-3 mt-1 mb-4 text-left">
            <span className={`w-5 h-5 rounded-[6px] border-2 flex items-center justify-center flex-none ${agree ? 'bg-[var(--accent)] border-[var(--accent)]' : 'border-[var(--line-2)]'}`}>
              {agree && <Check width={13} height={13} className="text-[var(--accent-ink)]" />}
            </span>
            <span className="text-[var(--ink-3)] text-[12.5px] font-semibold">I agree to the advertiser Terms &amp; no-spam policy</span>
          </button>
  ...
  return (
    <div className="mb-4">
      ...
      <input
        ...
        autoCapitalize="none"
        className="w-full bg-[var(--fill)] border border-[var(--line-2)] rounded-[14px] px-4 py-[13px] text-[var(--ink)] text-[15px] font-semibold placeholder:text-[var(--ink-5)] outline-none focus:border-[var(--accent)]/60"
      />
      {hint && <div className="text-[var(--ink-4)] text-[11px] font-semibold mt-1.5">{hint}</div>}
    </div>
  )
}
