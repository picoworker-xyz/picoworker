    ...
    <Page title="Add funds" subtitle="Send USDC on Solana to your deposit address." back narrow>
      <div className="rounded-[var(--r)] p-5 bg-[var(--card)] border border-[var(--line)] mb-4 text-center">
        ...
        <div className="flex items-center justify-center gap-4 mt-2 text-[12px] font-semibold">
          <span className="text-[var(--ink-4)]">Total {usd(bal?.balance ?? wallet?.business_escrow ?? 0)}</span>
          <span className="text-[#FFB05A]">Held {usd(bal?.held ?? 0)}</span>
        </div>
        {bal && bal.held > 0 && <div className="text-[var(--ink-4)] text-[11px] font-semibold mt-1">Held covers pending submissions and rejections under 7-day review.</div>}
        {(bal?.available ?? 0) > 0.2 && (
          <button onClick={() => nav('/wallet/withdraw?source=business')} className="mt-4 px-5 py-2.5 rounded-[12px] bg-[var(--fill)] text-[var(--ink)] text-[13px] font-extrabold font-head">
            Withdraw available
          </button>
        )}
      </div>
      ...
      {addrErr ? (
        ...
        <div className="rounded-[16px] p-6 bg-[var(--card)] border border-[var(--line)] flex items-center justify-center gap-3 text-[var(--ink-3)]">
          <div className="w-5 h-5 rounded-full border-2 border-[var(--line-2)] border-t-[var(--accent)] animate-spin" /> Loading your deposit address…
        </div>
      ) : (
        <>
          <div className="rounded-[18px] p-5 bg-[var(--card)] border border-[var(--line)]">
            <div className="text-[var(--ink-4)] text-[12px] font-bold uppercase tracking-[.07em] mb-3 text-center">Your USDC deposit address · Solana</div>
            ...
            <div className="flex items-center gap-2 rounded-[12px] bg-[var(--fill)] border border-[var(--line)] p-2 pl-3">
              <div className="flex-1 text-[var(--ink-2)] text-[12px] font-bold break-all font-head">{address}</div>
              <button onClick={() => { navigator.clipboard?.writeText(address).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 1500) }} className="px-3 py-2 rounded-[10px] bg-[var(--fill)] text-[var(--ink)] text-[12px] font-bold flex-none">{copied ? 'Copied!' : 'Copy'}</button>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 mt-4 rounded-[12px] bg-[rgba(255,176,90,.07)] border border-[rgba(255,176,90,.2)] p-3">
            ...
            <p className="text-[var(--ink-2)] text-[12px] font-semibold leading-[1.5]">
              Send <span className="font-extrabold text-[var(--ink)]">USDC on the Solana network only</span>. Other tokens or other chains will be lost. Your escrow is credited automatically once the transfer confirms.
            </p>
          </div>

          {success && <div className="text-[var(--green)] text-[13px] font-bold mt-3 text-center">{success}</div>}
          {msg && <div className="text-[var(--ink-3)] text-[12.5px] font-semibold mt-3 text-center">{msg}</div>}
          ...
          <div className="text-center text-[var(--ink-4)] text-[12px] font-semibold mt-3">We scan the Solana chain and credit any USDC received.</div>

          {/* Deposit history — every on-chain transfer, credited once by signature */}
          <div className="mt-7">
            <div className="text-[var(--ink)] text-[15px] font-extrabold font-head mb-3">Your deposits</div>
            {deposits.length === 0 ? (
              <div className="rounded-[14px] bg-[var(--card)] border border-[var(--line)] py-8 text-center text-[var(--ink-4)] text-[13px] font-semibold">
                No deposits yet. Sent USDC shows here once it confirms.
              </div>
            ) : (
              <div className="rounded-[16px] bg-[var(--card)] border border-[var(--line)] overflow-hidden">
                {deposits.map((d, i) => (
                  <div key={d.id} className={`flex items-center gap-3 px-4 py-[13px] ${i === 0 ? '' : 'border-t border-[var(--line)]'}`}>
