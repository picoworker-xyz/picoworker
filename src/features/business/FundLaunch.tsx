...
export function FundLaunch() {
  ...
  const t = task(taskId)
  if (!t || !wallet) {
    return <Page title="Review & fund" back><div className="text-center text-[var(--ink-4)] text-[14px] font-semibold py-16">Task not found.</div></Page>
  }
      ...
      <CenteredPage>
        <div className="rounded-[24px] bg-[var(--card)] border border-[var(--line)] p-8 text-center">
          <div className="flex flex-col items-center" style={{ animation: 'pico-pop .4s ease both' }}>
            ...
            <div className="font-head font-bold text-[24px] text-[var(--ink)] mt-6">Campaign is live!</div>
            <div className="text-[var(--ink-3)] text-[14px] font-semibold mt-2 leading-[1.5]">"{t.title}" is now visible to thousands of earners.</div>
          </div>
          <div className="flex flex-col gap-3 mt-8">
            ...
            <button onClick={() => nav('/business', { replace: true })} className="text-[var(--ink-3)] text-[14px] font-bold">Back to dashboard</button>
          </div>
        </div>
      </CenteredPage>
    )
  ...
  return (
    <Page title="Review & fund" subtitle="Step 2 of 2 · Confirm and launch" back narrow>
      ...
      <div className="text-[var(--ink)] text-[15px] font-extrabold font-head mb-3">Cost breakdown</div>
      <div className="rounded-[16px] p-4 bg-[var(--fill)] border border-[var(--line)] flex flex-col gap-[10px] mb-4">
      ...
      <div className="flex items-center justify-between rounded-[14px] p-4 bg-[var(--card)] border border-[var(--line)] mb-4">
        <div className="flex items-center gap-3">
          <span className="w-9 h-9 rounded-full bg-[var(--usdc)] flex items-center justify-center text-[#fff] text-[14px] font-extrabold">$</span>
          <div>
            <div className="text-[var(--ink)] text-[14px] font-bold">USDC balance</div>
            <div className="text-[var(--ink-4)] text-[12px] font-semibold">{usd(wallet.business_escrow)} available</div>
          </div>
        </div>
        {enough ? <Check width={20} height={20} className="text-[var(--green)]" /> : <button onClick={() => nav('/business/add-funds')} className="text-[var(--accent-strong)] text-[13px] font-extrabold">Add funds</button>}
      </div>

      <div className="flex items-center gap-3 px-4 py-4 rounded-[14px] bg-[var(--fill)] border border-[var(--line)] mb-2">
        <Shield width={18} height={18} className="text-[var(--green)] flex-none" />
        <span className="text-[var(--ink-3)] text-[12.5px] font-semibold">Funds sit in escrow. You only pay per verified completion — pause and refund unused budget anytime.</span>
      </div>
      ...
      <div className="text-center text-[var(--ink-4)] text-[13px] font-semibold mt-3">Goes live instantly to thousands of earners</div>
    </Page>
  )
}
...
function Line({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`font-head ${strong ? 'text-[var(--ink)] text-[16px] font-extrabold' : 'text-[var(--ink-2)] text-[14px] font-bold'}`}>{value}</span>
    </div>
  )
}
