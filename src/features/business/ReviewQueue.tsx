      ...
      <div className="max-w-[760px]">
        ...
        <div className="inline-flex bg-[var(--fill-2)] rounded-full p-1 mb-5">
          {(['pending', 'approved', 'rejected'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-full text-[13px] font-head capitalize ${tab === t ? 'bg-[var(--accent)] text-[var(--accent-ink)] font-extrabold' : 'text-[var(--ink-3)] font-bold'}`}>
          ...
          <div className="rounded-[var(--r)] border border-[var(--line)] bg-[var(--card)] py-14 text-center">
            {tab === 'pending' && (
              <>
                ...
                <div className="text-[var(--ink)] text-[15px] font-bold mt-3">All caught up</div>
                <div className="text-[var(--ink-4)] text-[13px] font-semibold mt-1">No proofs waiting on review.</div>
              </>
            )}
            {tab !== 'pending' && (
              <>
                <div className="text-[var(--ink)] text-[15px] font-bold">No {tab} proofs</div>
                <div className="text-[var(--ink-4)] text-[13px] font-semibold mt-1">No {tab} submissions found.</div>
              </>
            )}
          </div>
        ) : (
          <>
            ...
            <div className="rounded-[var(--r)] border border-[var(--line)] bg-[var(--card)] overflow-hidden">
              {filtered.map((p, i) => {
                ...
                return (
                  <button key={p.completion.id} onClick={() => nav(`/business/review/${p.completion.id}`)} className={`w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-[var(--fill)] ${i === 0 ? '' : 'border-t border-[var(--line)]'}`}>
                    ...
                    <span className="font-head text-[14px] font-extrabold text-[var(--accent-strong)]">{usd(p.completion.reward, { sign: true })}</span>
