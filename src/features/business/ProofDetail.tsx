...
export function ProofDetail() {
    ...
    return (
      <Page title="Proof" back narrow>
        <div className="rounded-[var(--r)] border border-[var(--line)] bg-[var(--card)] py-14 text-center">
          ...
          <div className="text-[var(--ink)] text-[15px] font-bold mt-3">Proof not found</div>
          <button onClick={() => nav('/business/review')} className="mt-5 font-head font-extrabold text-[14px] bg-[var(--fill)] text-[var(--ink)] px-5 py-3 rounded-[13px]">Back to queue</button>
        </div>
      </Page>
    )
  }
  ...
  return (
    <Page title="Proof Details" back narrow>
      ...
      {/* earner */}
      <div className="flex items-center gap-3 rounded-[16px] p-4 bg-[var(--card)] border border-[var(--line)] mb-4">
        <Avatar name={name} size={44} gradient="linear-gradient(135deg,#5B8DEF,#8B6CFF)" />
        <div className="flex-1">
          <span className="text-[var(--ink)] text-[15px] font-bold">{name}</span>
          <div className="text-[var(--ink-4)] text-[12px] font-semibold mt-[1px]">{tk?.title ?? 'Manual proof'}</div>
        </div>
        <div className="text-right">
          <div className="text-[var(--ink-4)] text-[11px] font-semibold uppercase">Reward</div>
          <div className="font-head text-[16px] font-extrabold text-[var(--accent-strong)]">{usd(completion.reward)}</div>
        </div>
      </div>
      ...
      {proofImages.length > 0 ? (
        <div className="rounded-[18px] border border-[var(--line)] bg-[var(--card)] p-4 mb-6">
        ...
        <div className="rounded-[18px] border border-[var(--line)] bg-[var(--card)] p-8 mb-6 flex flex-col items-center justify-center gap-2 text-[var(--ink-4)] text-[13px] font-semibold">
      ...
      {isPending ? (
        <>
          <div className="text-[var(--ink-3)] text-[12.5px] font-semibold mb-6 leading-[1.5]">
            Check the screenshot shows {tk?.title ?? 'the action'} completed by this username, then approve or reject.
          </div>
          {rejecting ? (
            <div className="flex flex-col gap-3">
              <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder="Reason for rejection (the worker will see this), e.g. the screenshot doesn't clearly show the follow." className="w-full bg-[var(--fill)] border border-[var(--line)] rounded-[14px] px-4 py-3 text-[var(--ink)] text-[14px] font-semibold placeholder:text-[var(--ink-5)] outline-none resize-none" />
              <div className="flex gap-3">
                <button onClick={() => setRejecting(false)} className="flex-1 font-head font-extrabold text-[15px] bg-[var(--fill)] text-[var(--ink)] py-[15px] rounded-[15px]">Cancel</button>
                <button onClick={() => decide(false, reason.trim() || undefined)} className="flex-[1.4] font-head font-extrabold text-[15px] bg-[var(--coral)] text-[#fff] py-[15px] rounded-[15px]">Confirm rejection</button>
              </div>
            </div>
          ) : (
          <div className="flex gap-3">
        ...
        <div className="text-[var(--ink-3)] text-[12.5px] font-semibold text-center py-4">
