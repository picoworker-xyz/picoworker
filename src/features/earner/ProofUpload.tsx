...
export function ProofUpload() {
  ...
  if (!t) return <Page title="Proof" back><div className="text-[var(--ink-4)] py-16 text-center">Task not found.</div></Page>
      ...
      <CenteredPage>
        <div className="rounded-[24px] bg-[var(--card)] border border-[var(--line)] p-8 text-center">
          <div className="flex flex-col items-center" style={{ animation: 'pico-pop .4s ease both' }}>
            ...
            <div className="font-head font-bold text-[24px] text-[var(--ink)] mt-6">Submitted for review</div>
            ...
            <div className="mt-5 px-4 py-2 rounded-full bg-[var(--fill)] text-[var(--ink)] text-[14px] font-bold font-head">{usd(earnerNet(t.reward), { sign: true })} pending approval</div>
          </div>
  ...
  return (
    <Page title="Submit proof" back narrow>
      <div className="flex items-center justify-between rounded-[18px] p-4 bg-[var(--card)] border border-[var(--line)] mb-6">
        <div>
          <div className="text-[var(--ink)] text-[15px] font-bold">{t.title}</div>
          <div className="text-[var(--ink-4)] text-[12px] font-semibold mt-1">Manual check · approved by the task giver</div>
        </div>
        <div className="font-head text-[18px] font-extrabold text-[var(--accent-strong)]">{usd(earnerNet(t.reward), { sign: true })}</div>
      </div>

      {/* what the task giver wants as proof */}
      {(specs.some((s) => s) || t.proof_instructions || (t.reference_images && t.reference_images.length > 0)) && (
        <div className="rounded-[16px] p-4 bg-[var(--card)] border border-[var(--line)] mb-5">
          <div className="text-[var(--ink-4)] text-[11px] font-bold uppercase tracking-[.07em] mb-2">Proof required ({need} screenshot{need > 1 ? 's' : ''})</div>
          {t.proof_instructions && <div className="text-[var(--ink-2)] text-[14px] font-semibold leading-[1.5] whitespace-pre-line mb-2">{t.proof_instructions}</div>}
          {specs.some((s) => s) && (
            <div className="flex flex-col gap-1.5">
              {Array.from({ length: need }, (_, i) => (
                <div key={i} className="flex items-start gap-2 text-[var(--ink-2)] text-[13.5px] font-semibold">
          ...
          {t.reference_images && t.reference_images.length > 0 && (
            <>
              <div className="text-[var(--ink-4)] text-[12px] font-semibold mt-3 mb-2">Example{t.reference_images.length > 1 ? 's' : ''}:</div>
              <div className="flex flex-wrap gap-2">
                {t.reference_images.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noreferrer" className="w-20 h-20 rounded-[10px] overflow-hidden border border-[var(--line-2)] block">
      ...
      {/* username / handle */}
      <div className="text-[var(--ink-4)] text-[12px] font-bold uppercase tracking-[.07em] mb-2">{handleLabel}</div>
      <input
        value={handle}
        onChange={(e) => setHandle(e.target.value)}
        placeholder={t.type === 'follow_x' ? '@yourhandle' : 'your username'}
        autoCapitalize="none"
        className="w-full bg-[var(--card)] border border-[var(--line-2)] rounded-[14px] px-4 py-[13px] text-[var(--ink)] text-[15px] font-semibold placeholder:text-[var(--ink-5)] outline-none focus:border-[var(--accent)]/60 mb-5"
      />

      {/* screenshots, one slot per required screenshot */}
      <div className="text-[var(--ink-4)] text-[12px] font-bold uppercase tracking-[.07em] mb-2">Your screenshot{need > 1 ? 's' : ''}</div>
          ...
          return (
            <div key={i}>
              ...
              {shot ? (
                ...
                <button onClick={() => openPicker(i)} className="w-full rounded-[16px] border border-dashed border-[var(--line-2)] bg-[var(--fill)] py-8 flex flex-col items-center justify-center gap-2 text-[var(--ink-3)] hover:bg-[var(--fill)]">
