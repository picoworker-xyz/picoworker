...
import { Bolt, ExternalLink, Star, Shield } from '../../components/icons'
...
import {
  ...
  detectCpaleadDevice,
  openCpaleadOffer,
  requestCpaleadOffers,
  type CpaleadOffer,
  type CpaleadState,
} from '../../lib/cpalead'

export function CpaleadOffers() {
  const [state, setState] = useState<CpaleadState>({ status: 'loading' })
  const [opening, setOpening] = useState<string | null>(null)
  const [err, setErr] = useState('')
  const device = useMemo(() => detectCpaleadDevice(), [])

  const load = useCallback(async (force = false) => {
    setState({ status: 'loading' })
    setState(await requestCpaleadOffers(device, { force }))
  }, [device])

  useEffect(() => {
    let active = true
    void requestCpaleadOffers(device).then((r) => { if (active) setState(r) })
    return () => { active = false }
  }, [device])

  async function open(offer: CpaleadOffer) {
    setErr('')
    ...
    try {
      const url = await openCpaleadOffer(offer, state.status === 'ready' ? state.country : null, device)
  ...
  return (
    <Page>
      ...
      <div className="mb-1 flex items-center gap-2 font-head text-[18px] font-extrabold text-[var(--ink)]">
        <Star width={19} height={19} className="text-[var(--accent-strong)]" /> Featured offers
      </div>
      ...
      {state.status === 'ready' && state.offers.length === 0 && (
        <div className="rounded-[22px] border border-[var(--line)] bg-[var(--card)] p-8 text-center">
          <Star width={30} height={30} className="mx-auto text-[var(--ink-5)]" />
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {state.offers.map((offer) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex h-11 w-11 flex-none items-center justify-center overflow-hidden rounded-[12px] bg-[var(--fill)]">
                    {offer.logo ? (
                      ...
                      <Star width={20} height={20} className="text-[var(--accent-strong)]" />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      <span className="text-[13px] font-extrabold text-[var(--green)]">
                        {cpaleadRewardLabel(offer)}
                      </span>
                      {/* CPAlead's own "fast pay" flag: these confirm in minutes
                          rather than hours, which is the single thing workers
                          ask about most. */}
                      {offer.fastPay && (
                        <span className="inline-flex items-center gap-0.5 rounded-full bg-[rgba(242,163,60,.14)] px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-[.04em] text-[#D99832]">
                          <Bolt width={9} height={9} /> Fast
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {(offer.requirement || offer.description) && (
