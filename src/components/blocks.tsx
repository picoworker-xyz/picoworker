  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start relative">
        <div>
          <div className="text-[var(--ink-4)] text-[12px] font-bold tracking-[.08em] uppercase mb-[7px]">Available balance</div>
          <div className="font-head font-bold text-[42px] text-[var(--ink)] tracking-[-.02em] leading-none">{usd(balance)}</div>
          <div className="flex items-center gap-[6px] mt-[9px]">
            ...
            <span className="text-[var(--ink-3)] text-[13px] font-semibold">≈ {balance.toFixed(2)} USDC</span>
          </div>
        </div>
        <button
      ...
      <div className="flex gap-[18px] mt-[18px] pt-[15px] border-t border-[var(--line)] relative">
...
function Stat({ value, label, accent }: { value: string; label: string; accent?: boolean }) {
  return (
    <div>
      <div className={`font-head text-[15px] font-extrabold ${accent ? 'text-[var(--accent-strong)]' : 'text-[var(--ink)]'}`}>{value}</div>
      <div className="text-[var(--ink-4)] text-[11px] font-semibold mt-[1px]">{label}</div>
    </div>
  )
}
const Divider = () => <div className="w-px bg-[var(--fill)]" />
      ...
      <div className="flex items-start gap-[13px] w-full">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-[7px]">
            <span className="text-[var(--ink)] text-[15px] font-bold leading-[1.25] line-clamp-2">{task.title}</span>
            {task.featured && <Pill tone="violet">FEATURED</Pill>}
          </div>
          <div className="text-[var(--ink-4)] text-[12px] font-semibold mt-[3px]">{meta}</div>
        </div>
        <div className="text-right flex-none">
          <div className="font-head text-[17px] font-extrabold text-[var(--accent-strong)]">{usd(earnerNet(task.reward), { sign: true })}</div>
        </div>
      </div>
      {goal > 0 && (
        <div>
          <div className="h-[5px] rounded-full bg-[var(--fill)] overflow-hidden">
            <div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${pct}%` }} />
          </div>
          <div className="text-[var(--ink-4)] text-[10.5px] font-semibold mt-1">{done.toLocaleString()} of {goal.toLocaleString()} done</div>
        </div>
      )}
    </button>
  )
