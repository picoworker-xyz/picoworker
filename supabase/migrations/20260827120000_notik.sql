...
-- ---------------------------------------------------------------------------
-- Replace the catalogue with a freshly pulled page set.
--
-- Upsert then delete-by-timestamp rather than truncate-then-insert: the table
-- stays readable throughout, so a visitor mid-refresh sees the old catalogue
-- instead of an empty page.
-- ---------------------------------------------------------------------------
create or replace function replace_notik_offers(p_offers jsonb)
returns int
language plpgsql security definer set search_path = public as $$
declare started timestamptz := now(); written int;
begin
  ...
  -- Anything the pull did not mention is disabled or expired. Sending traffic
  -- to it is explicitly unrewarded, so it must not survive a sync.
  delete from notik_offers where synced_at < started;

  update notik_sync
     set synced_at = now(), syncing_at = null, offers = written, error = null
   where id;
...
revoke all on function replace_notik_offers(jsonb) from public, anon, authenticated;
