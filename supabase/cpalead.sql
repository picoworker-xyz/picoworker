     ...
     and not exists (
       select 1 from cpalead_postbacks p
        where p.player_id = p_profile
          ...
          -- An event conversion does not finish the offer: one worker can
          -- complete several events on the same campaign, each paid as its own
          -- lead_id, so only a plain conversion (event_key blank) hides it.
